...
ComputePDGCGDamage::ComputePDGCGDamage(LAMMPS *lmp, int narg, char **arg) :
	...
	if (narg != 3)
		error->all(FLERR, "Illegal compute damage/atom command");
...
void ComputePDGCGDamage::init() {
	...
	for (int i = 0; i < modify->ncompute; i++)
		if (strcmp(modify->compute[i]->style, "pdgcg/damage") == 0)
			count++;
...
void ComputePDGCGDamage::compute_peratom() {
	...
	if (pair_type == NODES) {
		...
		partner = ((FixPDGCGShellsNeigh *) modify->fix[ifix_peri])->partner;
		npartner = ((FixPDGCGShellsNeigh *) modify->fix[ifix_peri])->npartner;
		vinter = ((FixPDGCGShellsNeigh *) modify->fix[ifix_peri])->vinter;
	}
	...
	for (i = 0; i < nlocal; i++) {
		if (mask[i] & groupbit) {
			...
			if (vinter[i] != 0.0)
				damage[i] = 1.0 - damage_temp / vinter[i];
			else
				damage[i] = 0.0;
