...
#include "string.h"
#include "compute_sph2_ulsph_stress.h"
#include "atom.h"
#include "update.h"
#include "modify.h"
#include "comm.h"
...
ComputeSph2ULSPHStress::ComputeSph2ULSPHStress(LAMMPS *lmp, int narg, char **arg) :
	...
	if (narg != 3)
		error->all(FLERR, "Illegal compute sph2/ulsph_stress command");
...
void ComputeSph2ULSPHStress::init() {
	...
	for (int i = 0; i < modify->ncompute; i++)
		if (strcmp(modify->compute[i]->style, "sph2/ulsph_stress") == 0)
			count++;
	if (count > 1 && comm->me == 0)
		error->warning(FLERR, "More than one compute sph2/ulsph_stress");
}
...
void ComputeSph2ULSPHStress::compute_peratom() {
	...
	int itmp = 0;
	Matrix3d *T = (Matrix3d *) force->pair->extract("sph2/ulsph/stressTensor_ptr", itmp);
	if (T == NULL) {
		error->all(FLERR,
				"compute sph2/ulsph_stress could not access stress tensors. Are the matching pair styles present?");
	}
