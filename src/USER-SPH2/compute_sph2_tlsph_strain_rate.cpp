...
#include "string.h"
#include "compute_sph2_tlsph_strain_rate.h"
#include "atom.h"
#include "update.h"
#include "modify.h"
#include "comm.h"
...
ComputeSph2TLSPHStrainRate::ComputeSph2TLSPHStrainRate(LAMMPS *lmp, int narg, char **arg) :
...
ComputeSph2TLSPHStrainRate::~ComputeSph2TLSPHStrainRate() {
	memory->sfree(strain_rate_array);
}

/* ---------------------------------------------------------------------- */

void ComputeSph2TLSPHStrainRate::init() {
...
void ComputeSph2TLSPHStrainRate::compute_peratom() {
	...
	int tlsph_flag = 0;
	int itmp = 0;
	Matrix3d *D = (Matrix3d *) force->pair->extract("sph2/tlsph/strain_rate_ptr", itmp);
	if (D == NULL) {
		error->all(FLERR,
				"compute sph2/tlsph_strain_rate could not access strain rate. Are the matching pair styles present?");
	}
...
double ComputeSph2TLSPHStrainRate::memory_usage() {
 ...
 */
Matrix3d ComputeSph2TLSPHStrainRate::Deviator(Matrix3d M) {
	Matrix3d eye;
	eye.setIdentity();
	eye *= M.trace() / 3.0;
	return M - eye;
