...
#undef ATOM_CLASS
		...
		error->all(FLERR, "Invalid atom style");
...
void Atom::tag_check() {
	...
	if (minall < 0)
		error->all(FLERR, "Atom ID is negative");
	if (maxall >= MAXTAGINT)
		error->all(FLERR, "Atom ID is too big");
	if (maxall > 0 && minall == 0)
		error->all(FLERR, "Atom ID is zero");
...
void Atom::deallocate_topology() {
	...
	atom->dihedral_atom1 = atom->dihedral_atom2 = atom->dihedral_atom3 = atom->dihedral_atom4 = NULL;
	...
	atom->improper_atom1 = atom->improper_atom2 = atom->improper_atom3 = atom->improper_atom4 = NULL;
}
...
void Atom::data_atoms(int n, char *buf) {
	...
	*next = '\n';
	...
	for (int i = 0; i < n; i++) {
		...
		if (coord[0] >= sublo[0] && coord[0] < subhi[0] && coord[1] >= sublo[1] && coord[1] < subhi[1] && coord[2] >= sublo[2]
				&& coord[2] < subhi[2])
...
void Atom::data_bonds(int n, char *buf, int *count) {
	...
	for (int i = 0; i < n; i++) {
		next = strchr(buf, '\n');
		*next = '\0';
		sscanf(buf, "%d %d " TAGINT_FORMAT " " TAGINT_FORMAT, &tmp, &itype, &atom1, &atom2);
		if (atom1 <= 0 || atom1 > map_tag_max || atom2 <= 0 || atom2 > map_tag_max)
...
void Atom::data_angles(int n, char *buf, int *count) {
	...
	for (int i = 0; i < n; i++) {
		next = strchr(buf, '\n');
		*next = '\0';
		sscanf(buf, "%d %d " TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT, &tmp, &itype, &atom1, &atom2, &atom3);
		if (atom1 <= 0 || atom1 > map_tag_max || atom2 <= 0 || atom2 > map_tag_max || atom3 <= 0 || atom3 > map_tag_max)
...
void Atom::data_dihedrals(int n, char *buf, int *count) {
	...
	for (int i = 0; i < n; i++) {
		...
		*next = '\0';
		sscanf(buf, "%d %d "
		TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT, &tmp, &itype, &atom1, &atom2, &atom3, &atom4);
		if (atom1 <= 0 || atom1 > map_tag_max || atom2 <= 0 || atom2 > map_tag_max || atom3 <= 0 || atom3 > map_tag_max
...
void Atom::data_bonus(int n, char *buf, AtomVec *avec_bonus) {
	...
	*next = '\n';
	...
	for (int i = 0; i < n; i++) {
		...
		if ((m = map(tagdata)) >= 0)
			avec_bonus->data_atom_bonus(m, &values[1]);
...
void Atom::data_bodies(int n, char *buf, AtomVecBody *avec_body) {
	...
	for (int i = 0; i < n; i++) {
		if (i == 0)
			tagdata = ATOTAGINT(strtok(buf, " \t\n\r\f"));
		else
			tagdata = ATOTAGINT(strtok(NULL, " \t\n\r\f"));
...
void Atom::set_mass(const char *str) {
	if (mass == NULL)
		error->all(FLERR, "Cannot set mass for this atom style");

	int itype;
	double mass_one;
	int n = sscanf(str, "%d %lg", &itype, &mass_one);
	if (n != 2)
		error->all(FLERR, "Invalid mass line in data file");
	...
	if (mass[itype] <= 0.0)
		error->all(FLERR, "Invalid mass value");
}
...
void Atom::check_mass() {
	...
	for (int itype = 1; itype <= ntypes; itype++)
		if (mass_setflag[itype] == 0)
			error->all(FLERR, "All masses are not set");
}
...
int Atom::shape_consistency(int itype, double &shapex, double &shapey, double &shapez) {
	...
	AtomVecEllipsoid *avec_ellipsoid = (AtomVecEllipsoid *) style_match("ellipsoid");
...
void Atom::add_molecule(int narg, char **arg) {
	...
	nmolecule += narg - 1;
	molecules = (Molecule **) memory->srealloc(molecules, nmolecule * sizeof(Molecule *), "atom::molecules");

	for (int i = 1; i < narg; i++) {
		molecules[nprevious] = new Molecule(lmp, arg[0], arg[i]);
		if (i == 1)
			molecules[nprevious]->nset = narg - 1;
		else
			molecules[nprevious]->nset = 0;
		nprevious++;
	}
}
...
int Atom::find_molecule(char *id) {
	...
	for (imol = 0; imol < nmolecule; imol++)
		if (strcmp(id, molecules[imol]->id) == 0)
			return imol;
	return -1;
}
...
void Atom::add_molecule_atom(Molecule *onemol, int iatom, int ilocal, tagint offset) {
	if (onemol->qflag && q_flag)
		q[ilocal] = onemol->q[iatom];
	if (onemol->radiusflag && radius_flag)
		radius[ilocal] = onemol->radius[iatom];
	if (onemol->rmassflag && rmass_flag)
		rmass[ilocal] = onemol->rmass[iatom];
	else if (rmass_flag)
...
void Atom::setup_sort_bins() {
	...
	if (userbinsize > 0.0)
		...
		if (domain->dimension == 3) {
			double vol = (domain->boxhi[0] - domain->boxlo[0]) * (domain->boxhi[1] - domain->boxlo[1])
					* (domain->boxhi[2] - domain->boxlo[2]);
			binsize = pow(1.0 * CUDA_CHUNK / natoms * vol, 1.0 / 3.0);
		} else {
			double area = (domain->boxhi[0] - domain->boxlo[0]) * (domain->boxhi[1] - domain->boxlo[1]);
...
void Atom::delete_callback(const char *id, int flag) {
	int ifix;
	for (ifix = 0; ifix < modify->nfix; ifix++)
		if (strcmp(id, modify->fix[ifix]->id) == 0)
			break;
